...
function normalizeCodexMatchText(text) {
  return String(text || '')
    .replace(/\s+/g, ' ')
    .trim()
    .toLowerCase();
...
function codexPaneMatchScore(filePath, paneText) {
  const pane = normalizeCodexMatchText(paneText);
  if (!pane) return 0;
    ...
    for (let i = lines.length - 1; i >= 0 && snippets.length < 6; i--) {
      try {
        ...
        for (const text of texts.reverse()) {
          const normalized = normalizeCodexMatchText(text);
...
function findCodexSessionFile(cwd, pid, args = '', procStartOverride = null, paneText = '') {
