...
function findCodexSessionFile(cwd, pid, args = '') {
  ...
  if (procStart) {
    ...
    const newSession = candidates
      ...
      .filter(f => (
        (Number.isFinite(f.birthtime) && f.birthtime >= procStart - 60000 && f.birthtime <= procStart + newSessionWindowMs) ||
        (Number.isFinite(f.metaTs) && f.metaTs >= procStart - 60000 && f.metaTs <= procStart + newSessionWindowMs)
      ))
      .sort((a, b) => a.score - b.score || a.birthtime - b.birthtime);
    if (newSession.length) return newSession[0].fp;

    const resumed = candidates
      .filter(f => f.birthtime < procStart && f.mtime >= procStart - 60000)
      .map(f => ({ ...f, firstTsAfterStart: firstCodexTimestampAfter(f.fp, procStart - 60000) }))
      .filter(f => Number.isFinite(f.firstTsAfterStart) && f.firstTsAfterStart <= procStart + 300000)
      .sort((a, b) => a.firstTsAfterStart - b.firstTsAfterStart);
    if (resumed.length) return resumed[0].fp;
