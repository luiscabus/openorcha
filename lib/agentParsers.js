  ...
  if (pid) {
    try {
      ...
      if (lstartRaw) {
        ...
        // 2. Older file that was modified after process start (resumed/continued)
        const resumed = files
          .filter(f => f.birthtime < procStart && f.mtime >= procStart)
          .sort((a, b) => {
            // Prefer the one whose mtime is closest to proc start (most likely the one it picked up)
            return Math.abs(a.mtime - procStart) - Math.abs(b.mtime - procStart);
          });
