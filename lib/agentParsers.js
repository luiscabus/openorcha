...
function findClaudeSessionFile(cwd, pid, args = '', procStartOverride = null, paneText = '', options = {}) {
  ...
  // Fallback (only when no pane text): file that was modified after process start
  // but created before it (auto-resumed / --continue session).
  if (pid) {
    try {
      ...
      if (Number.isFinite(procStart)) {
        ...
        //    IMPORTANT: Only use this heuristic when pane text is NOT available.
        //    When pane text was captured but didn't match, the "resumed" heuristic
        //    would incorrectly match files being actively written by OTHER PIDs.
        if (!hasPaneText) {
          const resumed = files
  ...
  // No pid provided and no pane text: fall back to most recently modified
  if (!hasPaneText) {
    files.sort((a, b) => b.mtime - a.mtime);
    return files[0].fp;
  }

  // Pane text was available but nothing matched — return null rather than
  // guessing wrong and showing another session's messages
  return null;
}
    ...
    try {
      ...
      if (msg.role === 'assistant') {
        if (msg.usage) {
          ...
          // Best-effort estimate of occupied context after the latest completed
          // assistant turn: prompt tokens sent plus the assistant output that
          // now also lives in conversation history.
          sessionMeta.lastContextTokens =
            usage.inputTokens +
            usage.cacheRead +
            usage.cacheCreation +
            usage.outputTokens;
...
function findCodexSessionFile(cwd, pid, args = '', procStartOverride = null, paneText = '', options = {}) {
  ...
  if (procStart) {
    ...
    // No confident match — return null rather than guessing wrong
    return null;
  }
